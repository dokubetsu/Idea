-- Drop existing function if any to support schema changes
...
-- Create atomic RPC function to wrap decision insertion, session update, and profile upsert with auth guard and secure search_path
...
BEGIN
  -- Verify p_user_id matches the authenticated user
  IF p_user_id <> auth.uid() THEN
    RAISE EXCEPTION 'Access denied. User ID mismatch.' USING ERRCODE = '42501';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Access denied. Session mismatch or unauthorized.' USING ERRCODE = '42501';
  END IF;
