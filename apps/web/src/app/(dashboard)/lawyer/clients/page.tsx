...
export default function LawyerClientsPage() {
  ...
  const perPage = 10;
  const { data: consultations = [], isLoading } = useConsultations({ page, per_page: perPage });
    ...
    <div className="animate-fade-in-up space-y-7">
      ...
      : consultations.length === 0 ? (
        <div className="space-y-6">
          <p className="py-16 text-center text-sm text-brand-blue-light/40">No requests yet.</p>
          {page > 1 && (
            <div className="flex items-center justify-between border border-brand-gold/12 bg-base-100 rounded-xl px-4 py-3">
              <button
                type="button"
                onClick={() => setPage(p => Math.max(1, p - 1))}
                className="rounded-lg border border-brand-gold/25 bg-base-100 px-3 py-1.5 text-xs font-semibold text-brand-blue-dark hover:bg-brand-gold/5 transition-all"
              >
                Previous
              </button>
      ...
      : (
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between border border-brand-gold/12 bg-base-100 rounded-xl px-4 py-3">
            ...
            <button
              type="button"
              disabled={consultations.length < perPage}
