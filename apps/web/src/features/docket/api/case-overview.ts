...
export const getCaseOverview = (matterId: string) =>
  apiClient.get<CaseOverview>(`/docket/matters/${matterId}/overview`);
...
export const createNote = (matterId: string, content: string) =>
  apiClient.post<InternalNote>(`/docket/matters/${matterId}/notes`, { content });

export const createTask = (matterId: string, data: { title: string; assigned_to?: string; due_date?: string }) =>
  apiClient.post<CaseTask>(`/docket/matters/${matterId}/tasks`, data);

export const updateTask = (matterId: string, taskId: string, data: { is_completed?: boolean }) =>
  apiClient.patch<CaseTask>(`/docket/matters/${matterId}/tasks/${taskId}`, data);
...
export const createTimelineEvent = (matterId: string, data: { event_type: string; lawyer_description: string; client_description?: string }) =>
  apiClient.post<TimelineEvent>(`/docket/matters/${matterId}/timeline`, data);
...
) => apiClient.post<Hearing>(`/docket/matters/${matterId}/hearings`, data);

export const listHearings = (matterId: string) =>
  apiClient.get<Hearing[]>(`/docket/matters/${matterId}/hearings`);

export const updateHearing = (
  matterId: string,
  hearingId: string,
  data: { status?: string; notes?: string; outcome?: string; next_date?: string }
) => apiClient.patch<Hearing>(`/docket/matters/${matterId}/hearings/${hearingId}`, data);

export const listDocuments = (matterId: string) =>
  apiClient.get<Document[]>(`/docket/matters/${matterId}/documents`);

export const reviewDocument = (
  matterId: string,
  docId: string,
  data: { status: string; lawyer_note?: string }
) => apiClient.patch<Document>(`/docket/matters/${matterId}/documents/${docId}/review`, data);

export const updateDocumentNote = (matterId: string, docId: string, lawyer_note: string) =>
  apiClient.patch<Document>(`/docket/matters/${matterId}/documents/${docId}/note`, { lawyer_note });

export const listMessages = (matterId: string) =>
  apiClient.get<Message[]>(`/docket/matters/${matterId}/messages`);

export const sendMessage = (
  matterId: string,
  data: { content: string; message_type?: string; attachment_path?: string }
) => apiClient.post<Message>(`/docket/matters/${matterId}/messages`, data);
...
export const listDocumentRequests = (matterId: string) =>
  apiClient.get<DocumentRequest[]>(`/docket/matters/${matterId}/document-requests`);

export const createDocumentRequest = (
  matterId: string,
  data: { title: string; description?: string; label: string }
) => apiClient.post<DocumentRequest>(`/docket/matters/${matterId}/document-requests`, data);

export const cancelDocumentRequest = (matterId: string, requestId: string) =>
  apiClient.patch<DocumentRequest>(`/docket/matters/${matterId}/document-requests/${requestId}/cancel`, {});
...
export const fulfillDocumentRequest = (
  ...
  return apiClient.postForm<DocumentRequest>(
