...
from app.domains.matters.schemas import (
    MatterOut, MatterUpdateRequest, PostUpdateRequest,
    UpdateOut, FactOut, VerifyFactRequest, AssignLawyerRequest,
    HearingOut, HearingCreate, HearingUpdate,
    MilestoneOut, MilestoneCreate, MilestoneUpdate,
    MatterCreateRequest, MeetingOut, MeetingCreate, MeetingUpdate
)
from app.domains.matters.service import enrich, get_matter_or_403, transition_status, SELECT
...
async def create_matter(body: MatterCreateRequest, user: LawyerOrAdmin):
    ...
    default_milestones = [
        {"title": "Intake & Consultation", "description": "Initial case assessment and client intake completed.", "order_index": 1, "status": "completed"},
        {"title": "Drafting Petition", "description": "Lawyer drafting case petition and reviewing files.", "order_index": 2, "status": "current"},
        {"title": "Filing Case", "description": "Filing case in the respective court/forum.", "order_index": 3, "status": "pending"},
        {"title": "Summons Issued", "description": "Court issuing summons to the opposing party.", "order_index": 4, "status": "pending"},
        {"title": "Arguments & Trial", "description": "Hearings for arguments, evidence, and trial details.", "order_index": 5, "status": "pending"},
        {"title": "Final Order / Judgment", "description": "Court pronouncing the final decision/order.", "order_index": 6, "status": "pending"},
    ]
    for dm in default_milestones:
        db.table("matter_milestones").insert({
            "matter_id": r["id"],
            **dm
        }).execute()
    ...
    milestones = db.table("matter_milestones").select("*").eq("matter_id", r["id"]).order("order_index").execute().data or []
...
async def get_matter(matter_id: str, user: Auth):
    ...
    milestones = db.table("matter_milestones").select("*").eq("matter_id", matter_id).order("order_index").execute().data or []
...
async def verify_fact(matter_id: str, fact_id: str, body: VerifyFactRequest, user: LawyerOrAdmin):
    ...
    await emit(EventType.FACT_VERIFIED, actor_id=user.id, matter_id=matter_id,
...
async def post_update(matter_id: str, body: PostUpdateRequest, user: Auth):
    ...
    full = db.table("matter_updates").select("*, profiles!author_id(full_name)").eq("id", r["id"]).single().execute().data
    author_name = (full.pop("profiles", None) or {}).get("full_name")
    
    await emit(EventType.UPDATE_POSTED, actor_id=user.id, matter_id=matter_id, 
...
async def assign_lawyer(matter_id: str, body: AssignLawyerRequest, user: Auth):
    ...
    await emit(EventType.LAWYER_ASSIGNED, actor_id=user.id, matter_id=matter_id,
               payload={"lawyer_id": body.lawyer_id, "direct": is_admin})
    return {"ok": True, "message": "Lawyer assigned" if is_admin else "Request sent to lawyer"}
...
async def create_hearing(matter_id: str, body: HearingCreate, user: LawyerOrAdmin):
    ...
    await emit(EventType.HEARING_SCHEDULED if body.status == "scheduled" else EventType.HEARING_UPDATED, 
               actor_id=user.id, matter_id=matter_id, payload={"hearing_id": r["id"]})
    return r
...
async def update_hearing(matter_id: str, hearing_id: str, body: HearingUpdate, user: LawyerOrAdmin):
    ...
    await emit(EventType.HEARING_UPDATED, actor_id=user.id, matter_id=matter_id, payload={"hearing_id": hearing_id})
    return r.data[0]
...
async def create_milestone(matter_id: str, body: MilestoneCreate, user: LawyerOrAdmin):
    ...
    r = db.table("matter_milestones").insert({
        "matter_id": matter_id,
        "title": body.title,
